...
switch($action)
{
	...
	{	
		...
		if ( ( isset($_POST['name']) === true && $_POST['name'] != NULL )  &&
	         ...
	         ( isset($_POST['type']) === true && $_POST['type'] != NULL )  &&
	         ( isset($_POST['url']) === true && $_POST['url'] != NULL )			  	   
		   )
		{				
			$ownerEl = checkVariable($_POST['ownerEl']);
			$slave = (int) checkVariable($_POST['slave']);
			$type = (int) checkVariable($_POST['type']);
			$name = checkVariable($_POST['name']);	
			$url = checkVariable($_POST['url']);			
		  
			$out = $treeManager->insertElement($name, $ownerEl, $slave, $type, $url);						
		}
		else {
			$out = FAILED; 
		}
    ...
    case "updateElementName":
    {
		...
		{			
			...
			$ownerEl = checkVariable($_POST['ownerEl']);
			$out = $treeManager->updateElementName($name, $elementId, $ownerEl, $url);
		}                         
		else {
			$out = FAILED;	
		}

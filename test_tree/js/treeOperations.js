...
function TreeOperations()
{
	...
	this.inputText = " <span class='typeLable'>title : </span><input type='text' id='inputText' maxlength='30'> <span class='typeLable'>url : </span> <input type='text' id='inputUrl' maxlength='60'> <span class='typeLable'>type : </span>" +
										"<select id='inputType'><option value='2'> Layer </option>" +
										"<option value='4'> Div </option>" +
										"<option value='5'> Drop </option>" +
										"<option value='6'> Dot </option>" +
										"<option value='7'> file </option>" +
										"<option value='8'> plugin </option>" +
										"</select>";
		...
		$('#inputText').bind("keypress", 
			 ...
			 {												
				...
				{	
					...
					var params = encodeURI("action=insertElement"+"&name="+name+"&ownerEl="+ownerEl+"&slave="+slave+"&type="+type+"&url="+url);
...
*******************************************************/
	this.trUpdateElementName = function(result)
	{
		...
		var tmp_node_url = "<span style='visibility:hidden;' id='url_"+info.elementId+"'>"+info.url+"</span>";
		...
		$('span#url_'+ elementId).replaceWith(tmp_node_url);
		$('#inputUrl').remove();
	...
	this.updateElementNameReq = function()
	{
		...
		// make ajax call to get all additional info to update
		var url = $('span#url_'+elementId).text();
		
		
		
		$('span:first', treeOps.trGetSelected()).replaceWith("<input type='text' id='inputText' value='"+elementName+"'/><input type='text' size='70' id='inputUrl' value='"+url+"'/>");	
		$('#inputText').focus();
		$('#inputText').bind("keypress",
								 function(evt)
								 {
									 if (evt.keyCode == 13) { //pressed enter
										...
										var url = $('#inputUrl').attr('value'); 
										var ownerEl = $('#inputText').parent().parent().parent().attr('id');
									 	var params = "action=updateElementName&name="+name+"&elementId="+elementId+"&ownerEl="+ownerEl+"&url="+url;
